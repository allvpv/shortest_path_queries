...
def get_edge_info(node1, node2, node_cache, parser, partitions):
    if node1 not in node_cache:
        return
...
def send_partition_edges(node_cache, partitions, parser):
    ...
    _lat, _lon = next(node_cache.values())
    ...
    for _, line in parser.get_lines():
        ...
        if way_begin.match(line):
            way_lines = [line]
            continue
        if not way_end.match(line):
            continue

        way_nodes, way_tags = parser.parse_way(way_lines)
        way_lines = None
        ...
        for node1, node2 in zip(way_nodes, way_nodes[1:]):
            edge_info = get_edge_info(node1, node2, node_cache, parser, partitions)
...
class ManagerServiceServicer(object):
    ...
    def GetGraphFragment(self, request, context):
        ...
        node_cache = parser.get_partition_nodes(partitions, partition_ix)
